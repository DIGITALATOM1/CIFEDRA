...
document.querySelector('#print-all').innerHTML = journeys.map(j => `<section><p>SUNLEO · Концепция MVP · 12.09.2026 · 1 площадка / 3 места</p><h2>${j.label}: ${j.title}</h2><p>${j.goal}</p><p>Мастер заранее выбирает ПВУ; SUNLEO задаёт цену и длительность; деньги резервируются. Момент резерва и списания уточняется. Без клиента и услуги мастер не платит.</p><table><thead><tr><th>Этап и действие</th><th>Ожидание / контакт</th><th>Трудность / ответ MVP</th><th>Результат</th></tr></thead><tbody>${j.steps.map((s,i) => `<tr><td><b>${i+1}. ${s[0]}</b><br>${s[1]}</td><td>${s[2]}<br><br>${s[3]}</td><td>${s[4]}<br><br>${s[5]}</td><td>${s[6]}</td></tr>`).join('')}</tbody></table><p>${j.boundary}</p><p>Открытые решения: тарифы, комиссия, выплаты, цена и длительность услуги, момент оплаты, возвраты, отмены, проверка мастеров, ресурсы, сроки и критерии успеха пилота.</p></section>`).join('');
